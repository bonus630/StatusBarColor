  ...
  <xsl:template match="uiConfig/items">
    <xsl:copy>
      <xsl:apply-templates select="node()|@*"/>
		...
		<!-- Creates a button to add at a menu, menu item needs works with DataSource-->
		<!--<itemData guid="28ef6590-9aa2-493a-9ddc-dd9955ed5ac3"
				   type="button"
				   onInvoke="*Bind(DataSource=StatusBarColorDS;Path=MenuItemCommand)"
				   caption="*Bind(DataSource=StatusBarColorDS;Path=Caption)"
				   icon="*Bind(DataSource=StatusBarColorDS;Path=Icon)"
				   enable="true"/>-->
		
		<!--Creates a item to add a Warpper, responsible to initialize the DataSource to be consumimed in menu item above -->
      <itemData guid="997e3380-dd9b-478e-97da-93b69ff0cb17"
                type="wpfhost"
                hostedType="Addons\StatusBarColor\StatusBarColor.CorelAddon,StatusBarColor.ControlUI"
                enable="true"/>
    ...
    <xsl:copy>
      <xsl:apply-templates select="node()|@*"/>
        ...
        <container>
          ...
          <item dock="fill" margin="0,0,0,0" guidRef="997e3380-dd9b-478e-97da-93b69ff0cb17"/>
        </container>
